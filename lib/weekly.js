...
function shiftWindow(window,days){const start=new Date(`${window.start}T00:00:00Z`),end=new Date(`${window.end}T00:00:00Z`);return{start:iso(new Date(start.valueOf()+days*DAY)),end:iso(new Date(end.valueOf()+days*DAY))}}
function previousWindow(window){return shiftWindow(window,-7)}
...
function averageWeeks(weeks){const n=weeks.length||1;return{offers:weeks.reduce((s,x)=>s+x.offers,0)/n,volume:weeks.reduce((s,x)=>s+x.volume,0)/n,leaders:weeks.reduce((s,x)=>s+x.leaders,0)/n,professionalShare:weeks.reduce((s,x)=>s+(Number.isFinite(x.professionalShare)?x.professionalShare:0),0)/n,top3Concentration:weeks.reduce((s,x)=>s+(Number.isFinite(x.top3Concentration)?x.top3Concentration:0),0)/n}}

export function buildWeeklyReport(items,reference=new Date(),options={}){
  const market=typeof options==='string'?options:(options.market||'FIDC');
  const period=previousFullWeek(reference),priorPeriod=previousWindow(period),current=summarizeWindow(items,period),prior=summarizeWindow(items,priorPeriod);
  const historyWeeks=Array.from({length:8},(_,i)=>summarizeWindow(items,shiftWindow(period,-7*(7-i))));
  const prior4Weeks=historyWeeks.slice(3,7),prior4Average=averageWeeks(prior4Weeks);
  const offerChangePct=pctChange(current.offers,prior.offers),volumeChangePct=pctChange(current.volume,prior.volume);
  const vs4OffersPct=pctChange(current.offers,prior4Average.offers),vs4VolumePct=pctChange(current.volume,prior4Average.volume);
  const top=current.topLeaders[0],observations=[];
  if(current.offers){
    ...
    if(Number.isFinite(vs4VolumePct))observations.push(`Contra a média das quatro semanas anteriores, o volume ficou ${Math.abs(vs4VolumePct).toFixed(1).replace('.',',')}% ${vs4VolumePct>=0?'acima':'abaixo'} e a quantidade de ofertas ${Math.abs(vs4OffersPct||0).toFixed(1).replace('.',',')}% ${(vs4OffersPct||0)>=0?'acima':'abaixo'}.`);
  ...
  return{
    generatedAt:new Date().toISOString(),market,current,prior,
    changes:{offersPct:offerChangePct,volumePct:volumeChangePct},
    benchmarks:{prior4Average,currentVsPrior4Average:{offersPct:vs4OffersPct,volumePct:vs4VolumePct}},
    history:{weeks:historyWeeks.map(x=>({period:x.period,offers:x.offers,volume:x.volume,leaders:x.leaders}))},
    observations,
    methodology:'Semana fechada de segunda a domingo. São consideradas as datas oficiais usadas pelo Radar: registro e, quando ausente, requerimento/protocolo. Volumes semanais somam somente valores positivos informados na base CVM. O benchmark de 4 semanas usa as quatro semanas completas anteriores à semana analisada. Sinais de tema, quando exibidos, usam apenas termos da denominação e são marcados como indicativos.'
  };
}

...
export function parseAnbimaIndicators(html){
  ...
  const updated=text.match(/Data e Hora da[\s\S]{0,40}?Atualiza.{0,8}o:\s*(\d{2}\/\d{2}\/\d{4})\s*-\s*([0-2]\d:[0-5]\d)/i);
  const selic=capture(text,/Estimativa\s+SELIC\s*\d*[^0-9]*(\d{2}\/\d{2}\/\d{4})\s+([\d.,]+)/i);
  const selicBc=capture(text,/Taxa\s+SELIC\s+do\s+BC\s*\d*[^0-9]*(\d{2}\/\d{2}\/\d{4})\s+([\d.,]+)/i);
  const di=capture(text,/DI-?B3\s*\d*[^0-9]*(\d{2}\/\d{2}\/\d{4})\s+([\d.,]+)/i);
  const usd=capture(text,/D\S*lar\s+Comercial\s+Venda\s*\d*[^0-9]*(\d{2}\/\d{2}\/\d{4})\s+([\d.,]+)/i);
  const ipcaProj=text.match(/IPCA\s*\d*[\s\S]{0,260}?Proje.{0,8}o\s*\([^)]*\)\s*([\d.,]+)/i);
  const igpmProj=text.match(/IGP-M\s*\d*[\s\S]{0,260}?Proje.{0,8}o\s*\([^)]*\)\s*([\d.,]+)/i);
  return {

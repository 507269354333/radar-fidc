...
export function parseAnbimaIndicators(html){
  ...
  const usd=capture(text,/D[oó]lar\s+Comercial\s+Venda\s*\d*[^0-9]*(\d{2}\/\d{2}\/\d{4})\s+([\d.,]+)/i);
  const ipcaProj=text.match(/IPCA\s*\d*[\s\S]{0,260}?Proje[cç][aã]o\s*\([^)]*\)\s*([\d.,]+)/i);
  const igpmProj=text.match(/IGP-M\s*\d*[\s\S]{0,260}?Proje[cç][aã]o\s*\([^)]*\)\s*([\d.,]+)/i);
  return {

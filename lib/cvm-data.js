...
let datasetCache=null,datasetCacheAt=0,datasetPending=null;
const DATASET_CACHE_MS=6*60*60*1000;
export async function fetchCvmMarketsDataset(fetchImpl=fetch){
 const cacheable=fetchImpl===fetch;
 if(cacheable&&datasetCache&&Date.now()-datasetCacheAt<DATASET_CACHE_MS)return datasetCache;
 if(cacheable&&datasetPending)return datasetPending;
 const load=async()=>{const {inputRows,sourceUpdatedAt}=await fetchInputRows(fetchImpl);return buildCvmMarketsDataset(inputRows,sourceUpdatedAt)};
 if(!cacheable)return load();
 datasetPending=load().then(data=>{datasetCache=data;datasetCacheAt=Date.now();return data}).finally(()=>{datasetPending=null});
